# Distributed Event Processing Platform — Design (v7)

v7: Incorporates review feedback from 2026-03-29-design-review-v6.md. See prior versions in 2026-03-29-event-platform-design-v6.md, 2026-03-29-event-platform-design-v5.md, 2026-03-29-event-platform-design-v4.md, 2026-03-29-event-platform-design-v3.md, 2026-03-29-event-platform-design-v2.md, and 2026-03-29-event-platform-design.md.
...
```python
{
    ...
    "timestamp": datetime,                  # indexed (client-provided, validated bounds)
    "user_id": "user_123",                 # indexed
    "source_url": "https://example.com",
    "metadata": { ... },                    # flexible JSON
    "created_at": datetime                  # set at enqueue time (service layer), not write time
}
```

`created_at` is set in the service layer when the event is enqueued and carried through the `QueueMessage` payload. This gives the field a consistent meaning ("when the system accepted the event") independent of queue depth. Under backpressure, events may sit in the queue before the worker persists them — setting `created_at` at enqueue time ensures the timestamp reflects acceptance, not persistence. This matters for TTL-based data retention (section 9, ARCHITECTURE.md): the expiry window starts from when the system took responsibility for the event, not from when the worker happened to process it.
...
- `EventCreate` — input validation: required `idempotency_key`, `event_type`, `timestamp`, `user_id`, `source_url`; optional `metadata` dict. Timestamp is validated with configurable bounds: rejected if more than a configurable window into the past (e.g., 30 days) or future (e.g., 5 minutes, accommodating client clock skew). This prevents pathological timestamps from creating sparse aggregation ranges or polluting realtime stats windows — same defensive-validation principle as metadata limits. Metadata is validated with defensive limits: maximum serialized size (e.g., 64KB) and maximum nesting depth (e.g., 10 levels). These prevent pathological inputs from causing disproportionate downstream effects (ES bulk write rejections, index bloat, expensive leaf-value extraction) without restricting legitimate use. Validation at the API boundary fails fast with a clear 422 before the event enters the async pipeline.
...
- `StatsQuery` — full analytics query: `time_bucket` enum (hourly/daily/weekly), optional `event_type` filter, date range. Date range is validated with maximum span per time bucket: hourly capped at 7 days, daily at 365 days, weekly at ~2 years. These prevent arbitrarily expensive aggregations — an unconstrained hourly query spanning a year would produce ~8,760 groups, consuming aggregation memory and time. Limits configurable via `config.py`. Rejected with 422. This applies the same parameter-constraining philosophy as the realtime endpoint (which uses window enums for cacheability) to the general stats endpoint (which uses it for predictable performance).
...
`QueueMessage` wraps the event payload with metadata: `message_id` (UUID), `retry_count`, `enqueued_at`, `last_error`. The event's `created_at` timestamp is set by the service layer at enqueue time and included in the payload — see section 3 for rationale.
...
10. Shutdown: sentinel value enqueued via lifespan shutdown hook using `asyncio.wait_for(queue.put(sentinel), timeout=N)`. If the put succeeds, the worker continues its normal drain-process loop until a drain result contains the sentinel. Items from that final drain that preceded the sentinel are processed as a normal batch. Then the worker exits. If the put times out, the worker is presumed dead — log a warning with the current queue depth (those events will be lost) and proceed with shutdown. The "no events lost during graceful shutdown" guarantee requires the worker to be alive and draining; if the worker has crashed, events in the in-memory queue are already lost regardless of the sentinel. SQS eliminates this concern entirely — shutdown means the worker stops polling, and unprocessed messages remain in the queue for redelivery.
...
- SQS eliminates the graceful shutdown sentinel concern — the worker simply stops polling, and unprocessed messages remain in the queue for redelivery. No sentinel, no timeout, no risk of shutdown hanging on a dead worker.
...
- `test_schemas.py` — Pydantic validation: required fields, type coercion, invalid input rejected. Covers both `EventCreate`/`EventAccepted` and the constrained `RealtimeStatsQuery`. Includes metadata validation: oversized payloads and deeply nested structures rejected with 422. Includes timestamp bounds validation: timestamps beyond the configured past/future window rejected with 422. Includes `StatsQuery` date range validation: date ranges exceeding per-bucket maximums rejected with 422.
...
- **Worker synchronization**: integration tests use `await queue.join()` after POSTing events to synchronize with the background worker. `asyncio.Queue.join()` blocks until all enqueued items have been `task_done()`'d — the worker already calls `task_done()` as part of `ack()`, so the join condition is naturally satisfied by the existing processing flow. This provides a precise, non-racy synchronization point with no polling or arbitrary sleeps. This is a test-only concern — SQS has no equivalent, but tests always need this kind of timing control.
...
| Worker dead + graceful shutdown | Sentinel enqueue times out. Events in the queue are lost. Shutdown proceeds with a warning log including queue depth. | Same as worker crash — in-memory queue contents are not recoverable without a live worker. SQS eliminates this — no sentinel needed, messages remain in queue for redelivery. |
...
- **MongoDB**: TTL index on `created_at` for automatic document expiry. Configurable retention period. `created_at` is set at enqueue time (when the system accepts the event), so TTL expiry is measured from acceptance, not from when the worker persisted it.
