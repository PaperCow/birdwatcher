# Distributed Event Processing Platform — Design (v3)

v3: Incorporates review feedback from 2026-03-29-design-review-v2.md. See prior versions in 2026-03-29-event-platform-design-v2.md and 2026-03-29-event-platform-design.md.
...
- If an item fails in either MongoDB or ES, it is nacked and re-enters the queue for retry. On retry, MongoDB `DuplicateKeyError` (code 11000) is treated as success — the event was already persisted, and the retry continues to the ES write. This is the expected retry path for ES-only failures. In production, CDC via change streams eliminates this dual-write retry concern entirely.
...
Application-level dual write from the worker. MongoDB is written first (source of truth), then ES is written for the items that succeeded. If ES fails for a given item, the item is nacked and retried through the main queue — the subsequent MongoDB `DuplicateKeyError` is treated as success, and ES gets another attempt. This means every ES retry incurs a redundant (but cheap) MongoDB unique index lookup.
...
6. Handle ES per-item results: ES failure for an item = nack (item re-enters queue; on next attempt, MongoDB dedup handles the re-insertion, ES gets another try).
7. Retryable failure (connection error, timeout): nack with exponential backoff (base 2s, max 60s, jitter).
...
10. Shutdown: sentinel value sent via lifespan shutdown, worker flushes remaining batch then exits.
...
- `test_queue.py` — Queue protocol: enqueue/drain ordering, drain respects max_items and timeout, nack re-enqueues with incremented retry count, max retries routes to DLQ, sentinel triggers shutdown.
- `test_worker.py` — Batch processing: batch fills to size, batch flushes on timeout, `BulkWriteError` handling (per-item classification by error code, only succeeded items forwarded to ES), ES failure triggers nack, retryable vs permanent failures, DLQ routing, DLQ overflow drops with logging.
...
Text/ASCII diagram from section 1 of this design, expanded with batch processing flow, per-item result handling, and health endpoint.
...
- **Worker**: batch consumption, bulk writes, `BulkWriteError` handling, retry/DLQ logic. Owns the write path to both stores.
...
| Elasticsearch down | ES portion of dual write fails; MongoDB writes succeed. Items nacked and retried — on retry, MongoDB dedup succeeds (cheap index lookup), ES gets another attempt. If ES stays down, items exhaust retries and route to DLQ. Search endpoint returns 503. | Search unavailable, all other endpoints functional. Health endpoint reports `degraded`. |
...
- **Queue**: SQS for durability, at-least-once delivery, and horizontal scaling. The `EventQueue` protocol abstraction makes this a swap, not a rewrite — `drain` maps to `receive_messages`, `ack` to `delete_message`, `nack` to `change_message_visibility(0)`.
