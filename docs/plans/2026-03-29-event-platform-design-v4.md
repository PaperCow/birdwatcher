# Distributed Event Processing Platform — Design (v4)

v4: Incorporates review feedback from 2026-03-29-design-review-v3.md. See prior versions in 2026-03-29-event-platform-design-v3.md, 2026-03-29-event-platform-design-v2.md, and 2026-03-29-event-platform-design.md.
...
- If an item fails in MongoDB (non-duplicate), it is nacked or routed to DLQ based on retry count. If an item fails in ES, the failure is classified by status code: 429/5xx are retryable (nack), 4xx (except 429) are permanent (DLQ immediately). Nacked items re-enter the queue for retry. On retry, MongoDB `DuplicateKeyError` (code 11000) is treated as success — the event was already persisted, and the retry continues to the ES write. This is the expected retry path for ES-only retryable failures. In production, CDC via change streams eliminates this dual-write retry concern entirely.
...
5. Bulk index succeeded items (including deduplicated) to ES (`async_bulk`). Items that failed MongoDB with non-duplicate errors are excluded from the ES write.
6. Handle ES per-item results by status code: success = ack. Retryable failure (429, 5xx) = nack (item re-enters queue; on next attempt, MongoDB dedup handles the re-insertion, ES gets another try). Permanent failure (4xx except 429, e.g., mapping conflict) = route to DLQ immediately, no retry — the event is persisted in MongoDB (source of truth intact), and retrying won't fix the ES rejection.
...
- SQS handles redelivery timing natively via visibility timeout — the per-batch-cycle backoff sleep in the in-memory implementation is not needed with SQS.
...
- `degraded`: ES or Redis unreachable (search, cache, or rate limiting unavailable, but core ingestion and analytics functionality works).
- `unhealthy`: MongoDB unreachable — the only dependency whose loss prevents the system's primary function.
- Used as the app's docker-compose health check.
- In a production Kubernetes deployment, this would be split into separate liveness (cheap, no dependency checks — just return 200) and readiness (dependency connectivity — check MongoDB; ES/Redis failures handled at the application level with per-endpoint 503s) probes. A single combined endpoint is sufficient for this scope. The `degraded`/`unhealthy` classification aligns with the Kubernetes model: `unhealthy` (MongoDB) would fail readiness (pod removed from rotation), while `degraded` (ES/Redis) would still pass readiness (pod continues serving traffic).
...
- Worker handles `DuplicateKeyError` (code 11000 in `BulkWriteError.details['writeErrors']`) as a successful outcome — the event was already persisted. The item proceeds to the ES bulk write (in case the prior attempt's ES write failed).
...
- `test_schemas.py` — Pydantic validation: required fields, type coercion, invalid input rejected. Covers both `EventCreate`/`EventAccepted` and the constrained `RealtimeStatsQuery`.
- `test_queue.py` — Queue protocol: enqueue/drain ordering, drain respects max_items and timeout, nack re-enqueues with incremented retry count, max retries routes to DLQ, sentinel triggers shutdown after processing preceding items.
- `test_worker.py` — Batch processing: batch fills to size, batch flushes on timeout, `BulkWriteError` handling (per-item classification by error code, only succeeded items forwarded to ES), ES per-item failure classification (retryable 429/5xx triggers nack, permanent 4xx triggers DLQ), batch-cycle backoff (consecutive failure counter increments/resets), DLQ routing, DLQ overflow drops with logging.
...
- `test_health.py` — Health endpoint returns correct status for healthy, degraded (ES down, Redis down), and unhealthy (MongoDB down) states.
...
Text/ASCII diagram from section 1 of this design, expanded with batch processing flow, per-item result handling (including ES status code classification), and health endpoint.
...
- **Worker**: batch consumption, bulk writes, `BulkWriteError` handling, ES status code classification, batch-cycle backoff, retry/DLQ logic. Owns the write path to both stores.
...
| Elasticsearch down | ES portion of dual write fails with 5xx; MongoDB writes succeed. Items nacked and retried — on retry, MongoDB dedup succeeds (cheap index lookup), ES gets another attempt. If ES stays down, items exhaust retries and route to DLQ. Search endpoint returns 503. | Search unavailable, all other endpoints functional. Health endpoint reports `degraded`. |
...
- **Health probes**: split into Kubernetes liveness (no dependency checks) and readiness (check MongoDB; ES/Redis handled with per-endpoint 503s) probes.
