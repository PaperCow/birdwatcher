# Distributed Event Processing Platform — Design (v5)

v5: Incorporates review feedback from 2026-03-29-design-review-v4.md. See prior versions in 2026-03-29-event-platform-design-v4.md, 2026-03-29-event-platform-design-v3.md, 2026-03-29-event-platform-design-v2.md, and 2026-03-29-event-platform-design.md.
...
6. Handle ES per-item results by status code: success = ack. Retryable failure (429, 5xx) = nack (item re-enters queue; on next attempt, MongoDB dedup handles the re-insertion, ES gets another try). Permanent failure (4xx except 429, e.g., mapping conflict) = route to DLQ immediately, no retry — the event is persisted in MongoDB (source of truth intact), and retrying won't fix the ES rejection. Note: if nack fails due to a full queue, the item is routed to the DLQ (see In-Memory Implementation).
